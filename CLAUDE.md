...
- **10 modular files** loaded via `<script>` tags in shared global scope:
  `hod-core.js` → `hod-fx.js` → `hod-shader-fx.js` → `hod-overlay.js` → `hod-audio.js` → `hod-timeline.js` → `hod-mask.js` → `hod-tracking.js` → `hod-hands.js` → `hod-region-fx.js` (entry: `index.html`)
